...
describe("Clients Route", () => {
  describe("Id Route", () => {
    it("Id - 401 No cookie", async () => {
      const res = await request.get(
        "/api/clients/id/a0ece5db-cd14-4f21-812f-966633e7be86"
      );
    ...
    it("Id - 403 Forbidden", async () => {
      const res = await request
        ...
        .set("Cookie", setCookie("client"));
      expect(res.statusCode).toBe(403);
    });

    it("Id - 404 - Id not found", async () => {
      const res = await request
        .get("/api/clients/id/a0ece5db-cd14-4f21-812f-966633e7be89")
        .set("Cookie", setCookie("user"));
      expect(res.statusCode).toBe(404);
    });

    it("Id - 200 as User", async () => {
      const res = await request
        .get("/api/clients/id/a74c83c5-e271-4ecf-a429-d47af952cfd4")
        .set("Cookie", setCookie("user"));
  ...
  describe("Name Route", () => {
    it("Name - 401 No cookie", async () => {
      const res = await request.get("/api/clients/name/britney");
      expect(res.statusCode).toBe(401);
    });

    it("Name - 403 Forbidden", async () => {
      const res = await request
        .get("/api/clients/name/britney")
        .set("Cookie", setCookie("client"));
      expect(res.statusCode).toBe(403);
    });
    ...
    it("Name - 404 - Id not found", async () => {
      const res = await request
        .get("/api/clients/name/britneyy")
        .set("Cookie", setCookie("user"));
      expect(res.statusCode).toBe(404);
    });

    it("Name - 200 as User", async () => {
      const res = await request
        .get("/api/clients/name/britney")
        .set("Cookie", setCookie("user"));
      expect(res.statusCode).toBe(200);
